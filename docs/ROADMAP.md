...
3. **작업 구현**
   - 작업 파일의 명세서를 따름
   - 기능과 기능성 구현
   - **🚨 테스트 필수 프로세스 (구현 → 테스트 → 검증 → 진행)**:
     - 각 기능 구현 후 **즉시** Playwright MCP로 테스트 수행
     - API 엔드포인트: 요청/응답 검증, 에러 핸들링 테스트
     - 비즈니스 로직: 정상 케이스 + 엣지 케이스 테스트
     - UI 연동: 사용자 플로우 E2E 테스트
     - **테스트 통과 전까지 해당 구현은 미완료 상태로 유지**
