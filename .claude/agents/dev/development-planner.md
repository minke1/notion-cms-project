...
description: Use this agent to create, update, or maintain ROADMAP.md files in Korean. Handles roadmap creation, adding development phases, updating task statuses, and organizing priorities.
model: opus
color: red
---
...
- 수락 기준과 완료 조건 정의
- **🧪 테스트 필수 원칙**:
  - API 연동, 비즈니스 로직 구현 시 **반드시** Playwright MCP를 활용한 테스트 수행
  - 구현 완료 후 테스트를 통과해야만 다음 단계로 진행 가능
  - 테스트 실패 시 수정 → 재테스트 반복
  - 모든 테스트 결과는 작업 파일에 기록
...
### 🧪 테스트 가이드라인 (Playwright MCP 필수)

#### **테스트 원칙**
1. **구현 즉시 테스트**: 코드 작성 후 바로 테스트 수행
2. **테스트 통과 필수**: 테스트 실패 시 다음 단계 진행 금지
3. **결과 기록**: 모든 테스트 결과를 작업 파일에 문서화

#### **테스트 대상별 시나리오**

| 구현 유형 | 테스트 항목 | Playwright MCP 활용 |
|----------|------------|-------------------|
| API 엔드포인트 | 요청/응답 검증, 상태 코드, 에러 핸들링 | `browser_navigate`, `browser_snapshot` |
| 비즈니스 로직 | 정상 케이스, 엣지 케이스, 예외 처리 | `browser_fill_form`, `browser_click` |
| 사용자 플로우 | 전체 시나리오 E2E 테스트 | `browser_snapshot`, `browser_take_screenshot` |
| 폼 제출 | 유효성 검증, 성공/실패 케이스 | `browser_fill_form`, `browser_type` |

#### **테스트 체크리스트 템플릿**

작업 파일에 다음 형식의 테스트 체크리스트를 **필수** 포함:
...
- [ ] **모든** API 연동 Task에 Playwright MCP 테스트가 포함되었는가?
- [ ] **모든** 비즈니스 로직 구현 Task에 테스트 시나리오가 정의되었는가?
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 **필수** 포함되었는가?
- [ ] 테스트 통과 기준이 명확히 정의되었는가?
- [ ] 정상 케이스 + 에러 케이스 + 엣지 케이스가 모두 포함되었는가?
- [ ] E2E 테스트 시나리오가 사용자 플로우별로 정의되었는가?
- [ ] Phase 3에 통합 테스트 Task가 포함되었는가?
- [ ] **테스트 미통과 시 진행 불가** 원칙이 명시되었는가?
